...
14. [架构回溯机制](#14-架构回溯机制architectural-back-propagation)
15. [需求生命周期管理](#15-需求生命周期管理requirement-lifecycle-management)
16. [原子变更快速通道](#16-原子变更快速通道atomic-change-fast-track)
17. [大型项目的上下文管理](#17-大型项目的上下文管理context-management-for-large-projects)
18. [紧急豁免协议](#18-紧急豁免协议emergency-waiver-protocol)
...
### 1.2.3 人类优先原则（Human-First Principle）

**场景**：在某些高度创意的设计场景或极其复杂的系统中，人类架构师可能想**亲自撰写 SDD-1 和 SDD-2** 文档，而不是由 VA 生成。

**AI 的行为准则**：

✅ **当人类提供了完整的 SDD 文档时**：
1. AI 应直接进入 CR（合规审查）和 CR_Code（代码审查）阶段
2. **不要试图重写、优化或"改进"人类的设计**
3. 只有在发现明显的**逻辑漏洞**或**安全风险**时才提出修改建议
4. 尊重人类的架构决策，即使与 AI 的风格不同

❌ **严禁的行为**：
- "我觉得这个设计可以更优化..." → 除非有客观缺陷，否则不主动建议修改
- 在不告知用户的情况下改动人类撰写的 SDD
- 声称 AI 生成的文档"更专业"而贬低人类的工作

**示例对话**：

```
【场景：人类提供了完整的 SDD-1/SDD-2】

用户（架构师）：我已经完成了支付模块的 SDD-1 和 SDD-2。
                请你作为 CR，检查文档的完整性和合规性。

AI VA：✅ 理解。我进入 CR 角色，尊重你的设计。

【检查结果】
- ✅ RQ-ID 定义清晰
- ✅ 测试计划完整（覆盖率目标 ≥ 85%）
- ⚠️ 性能 Benchmark 缺少具体数值（应补充）
- ❌ 缺少对 PCI DSS（支付卡数据标准）的合规说明

【合规结论】：部分合规 - 需在提交前补充上述两项

【AI 的态度】：我不改你的设计架构，只指出客观的缺陷。
             这是尊重人类专业性的表现。
```

**何时人类文档优于 AI 文档**：
- 涉及复杂的跨团队协作（人类可能更懂组织政治）
- 涉及外部合规要求（如 GDPR、PCI DSS）
- 涉及遗留系统集成（人类对历史背景理解更深）
- 涉及战略性决策（人类的商业直觉可能超越数据分析）
...
## 14. 架构回溯机制（Architectural Back-propagation）

> **目的**：解决编码阶段发现设计缺陷时的文档同步问题，防止"文档腐烂"和"代码漂移"。

### 14.1 触发条件

当开发者或 CR_Code 在编码/审查阶段发现以下情况时，**必须触发架构回溯**：

| 情况 | 说明 | 严重级别 |
|-----|------|---------|
| **接口变更** | 函数参数、返回值需要修改，与 SDD-2 定义不符 | 🔴 High |
| **算法不可行** | SDD-2 设计的算法在实际数据下无法跑通 | 🔴 High |
| **性能塌陷** | 按照设计实现的 Benchmark 远低于预期 | 🟡 Medium |
| **数据结构变更** | 需要调整核心数据结构，影响 RQ-ID 的实现映射 | 🔴 High |
| **安全漏洞** | 设计中遗漏的安全隐患在代码审查时发现 | 🔴 High |
| **可维护性问题** | 原设计导致代码复杂度过高或耦合度过强 | 🟡 Medium |
...
发现问题：函数签名变更
- 位置：src/auth/oauth.go, line 142
- 原设计：func ValidateToken(token string) bool
- 实际实现：func ValidateToken(ctx context.Context, token string) (bool, error)
  需要添加 context 和 error 处理，SDD-2 中未体现。
...
已更新 SDD-2：
✅ § 2.3 接口定义 - ValidateToken() 增加了 context 参数和 error 返回值
✅ § 3 测试计划 - 添加了 context.WithTimeout 的边界测试
✅ 变更日志 - 标记为"基于实现反馈的设计修正"

原因说明：
- 原设计未考虑超时场景
- 实现阶段发现没有 error 处理机制会导致静默失败
- 本修正符合 § 1.3 原则 4（安全优先）
...
## 15. 需求生命周期管理（Requirement Lifecycle Management）

> **目的**：规范需求从"创建 → 活跃 → 废弃 → 清理"的完整生命周期，防止"僵尸代码"和"孤立文档"。
...
### 15.2 废弃流程（Deprecation Process）

当需求不再需要或功能下线时，**不能直接删除**，必须走完整的废弃流程：
...
需求：RQ-005 旧版登录接口
状态：MARKED_FOR_DEPRECATION
理由：迁移到了 OAuth 2.0，旧接口不再使用
过期时间：T + 2 周（提前通知期）

需要做的事情：
- 在代码中添加 @Deprecated 注释
- 在 SDD-1 中标记 [DEPRECATED]
- 通知使用方进行迁移
```
...
□ 删除 SDD-1/RQ-005.md
□ 删除 SDD-2/RQ-005.md
□ 删除对应的代码文件或模块（internal/rpc/legacy_auth/）
□ 删除关联的单元测试和集成测试
□ 更新 API 文档，移除旧端点
□ 如有依赖关系，更新 SYSTEM_MAP.md

Commit 信息：
[DEPRECATION] Remove RQ-005 (legacy login API) after 2-week notice period
- Removed SDD documentation
- Deleted internal/rpc/legacy_auth module (1200 LOC)
- Updated API documentation
- Removed 45 test cases
```
...
✅ 正确做法：
1. 询问用户：这是废弃需求还是需求变更？
2. 建议走废弃流程（给系统使用方 2 周过渡期）
3. 输出完整的清理清单（包括代码、文档、测试）
4. 验证是否有其他需求对 RQ-005 有依赖
5. 提醒 ADMIN 需要在项目日志中记录这次退役
```
...
□ 检查是否有 @Deprecated 的代码存在超过 3 个月
□ 检查 inboxes/ 中是否有已关闭的但没清理的 RQ
□ 检查 sdd-2/ 中是否有对应 RQ 已删除的孤立文档
□ 检查代码中的 TODO/FIXME 注释，是否关联到活跃的 RQ-ID
□ 统计"清理对象"数量，如果持续增加，说明前期分析不足

如发现问题，纳入下个 Sprint 的 P3 任务。
```

---

## 16. 原子变更快速通道（Atomic Change Fast Track）

> **目的**：对于低风险、小范围的变更，提供轻量化的快速审批路径，避免流程繁琐导致团队抵触规范。
...
| 条件 | 说明 |
|-----|------|
| **API 签名不变** | 不改变函数/接口的参数、返回值 |
| **数据结构不变** | 不涉及数据库 schema、消息格式、存储结构的改动 |
| **预估工时** | < 1 小时（单个开发者） |
| **影响范围** | 只涉及 1-2 个模块内部 |
| **无依赖影响** | 不影响其他 RQ-ID 的实现 |
| **测试可覆盖** | 核心逻辑的测试可在 30 分钟内补全 |
| **文件数量限制** | 修改文件数 ≤ 3 个（防止用小变更掩盖大改动） |
| **目录限制** | 不涉及核心架构层（如 `core/`, `auth/`, `config/`, `infra/`, `db/schema/` 等关键目录） |

**⚠️ 防滥用机制**：

如果开发者试图将一个大功能拆成多个"原子变更"来规避审查，CR 应：
1. 检查是否涉及相关 RQ-ID（多个 RQ-ID 联动 = 不是原子变更）
2. 要求提供原始需求文档（完整还是碎片化？）
3. 如果怀疑滥用，转为正式 SDD 流程

### 16.2 快速通道流程

**原子变更不走完整的 SDD-0 → 1 → 2 流程，而是合并为单步：**
...
**文件名**：`sdd-atomic/ATOMIC-YYYYMMDD-NNN.md`

```markdown
# 原子变更：XXX

**RQ-ID**：临时使用 ATOMIC-20250101-001（不纳入正式 RQ 管理）
**预估工时**：30 分钟
**优先级**：P3（不影响其他功能）

## 1. 需求陈述

描述要修改什么、为什么要修改（1-2 段，控制在 100 字以内）。

示例：
修复登录界面的验证错误提示显示不全问题。
原因：错误文本长度超过了 UI 容器宽度。

## 2. 实现方案

直接给出解决方案，不需要列举多个候选方案。

示例：
将 ErrorMessage 组件的 max-width 从 200px 调整为 400px，
并在长度超过 300 字符时启用 text-wrap。
...
示例：
□ TestErrorMessageWraps_ShortText - 短错误提示不换行
□ TestErrorMessageWraps_LongText - 长错误提示自动换行
□ TestErrorMessageWraps_MaxWidth - 容器宽度限制生效
...
- ✅ 无依赖影响
- ✅ 测试覆盖率 ≥ 80%
- ✅ 安全性：无新的 UI injection 风险
```

### 16.4 原子变更的约束

**即使是快速通道，以下项目也 NOT 可跳过：**

- ❌ **安全检查**：任何涉及用户输入的变更，必须检查 XSS/CSRF
- ❌ **测试覆盖率**：必须 ≥ 80%
- ❌ **代码审查**：CR_Code 仍需审查
- ❌ **文档更新**：README 或配置文件需要更新时，必须更新

**只能跳过：**
- ✅ 需求分析（直接给出方案）
- ✅ 方案对比（不需要列举多个选项）
- ✅ 详细的 Benchmark（工时 < 1h，无需性能测试）

### 16.5 原子变更退级机制

如果在实现过程中发现原子变更的假设不成立（比如发现 API 需要改），**立即升级为正式 RQ**：

```
【CR_Code 或 DEV】

发现：实现过程中需要改变 API 签名
行动：立即转换为正式 SDD-0 → 1 → 2 流程

标记方式：
- 在 commit 中标注 [ATOMIC-ESCALATED-20250101-001]
- 将原子变更文档存档
- 创建正式的 RQ-XXX
```
...
## 17. 大型项目的上下文管理（Context Management for Large Projects）

> **目的**：当项目 docs/ 目录变得很大时，指导如何有效管理 AI 上下文窗口，避免加载延迟或精度丢失。
...
| 现象 | 表现 |
|-----|------|
| **加载缓慢** | 告诉 AI 读取 docs/ 时，API 响应延迟 > 10 秒 |
| **回答不精准** | AI 遗漏了文档中的关键细节 |
| **频繁超过限额** | 项目文档 > 20 MB（即便 Claude 支持 200k tokens） |
| **维护困难** | 无法快速定位相关的 SDD 文件 |

### 17.2 上下文分层加载策略

**不要一次性加载所有 SDD 文件，而是分层加载：**

#### 第一层：导航地图（必加载）
```
文件：docs/sdd/SYSTEM_MAP.md
大小：< 10 KB
内容：系统模块关系、RQ-ID 索引、文件导航

示例：
# 系统地图（SYSTEM_MAP.md）

## 模块关系
auth (RQ-001 ~ RQ-005) → user (RQ-010 ~ RQ-015) → payment (RQ-020 ~ RQ-025)

## RQ-ID 文档索引
| RQ-ID | 模块 | 文件 | 状态 |
|-------|------|------|------|
| RQ-001 | auth | sdd-1/RQ-001-oauth.md | ACTIVE |
| RQ-010 | user | sdd-1/RQ-010-user-profile.md | ACTIVE |
| RQ-020 | payment | sdd-1/RQ-020-payment-gateway.md | ACTIVE |
```

#### 第二层：相关模块（有需要时加载）
```
告诉 AI 只读取某个 RQ 的相关文件：

"我要设计 RQ-021（支付订单），请读取：
- SYSTEM_MAP.md（整体导航）
- sdd-1/RQ-021-order.md（需求规格）
- sdd-2/RQ-021-order.md（实现设计）
- sdd-1/RQ-020-payment-gateway.md（依赖的 RQ）"
```

#### 第三层：通用规范（按需加载）
```
文件：docs/sdd/COMMON_PATTERNS.md
内容：常用的设计模式、工具配置、最佳实践参考

目的：避免在每个 SDD 文件中重复规范说明。
```
...
```
docs/sdd/
├── SYSTEM_MAP.md              ← 导航文件（首先加载）
├── COMMON_PATTERNS.md          ← 通用模式库
│
├── sdd-1/                      ← 按模块分组
│   ├── auth/
│   │   ├── RQ-001-oauth.md
...
#### ❌ 错误做法：
```
"请读取 docs/sdd 目录中的所有文件，然后设计 RQ-025"
```
...
[大型项目上下文加载]
项目：Payment System
模块：Payment Processing

【基础加载】
- docs/sdd/SYSTEM_MAP.md
- docs/sdd/COMMON_PATTERNS.md

【需求相关】
- docs/sdd/sdd-1/payment/RQ-025-refund.md
- docs/sdd/sdd-1/payment/RQ-020-gateway.md （依赖）

【设计相关】
- docs/sdd/sdd-2/payment/RQ-025-refund.md
- docs/sdd/sdd-2/payment/RQ-020-gateway.md

我要设计 RQ-025（退款流程）的实现。
```

### 17.5 定期维护与自动化

#### 17.5.1 SYSTEM_MAP.md 自动维护（强制动作）

**当前痛点**：如果 VA 完成了 SDD-2 文档但忘了更新 SYSTEM_MAP.md，AI 导航会失效。
...
每次完成或更新 SDD-2 文档时，VA 必须：

1. 检查 docs/sdd/SYSTEM_MAP.md 是否存在
   - 如果不存在，创建该文件
   - 如果存在，进行更新

2. 自动添加或更新以下内容：
   - RQ-ID 的模块映射（RQ-XXX → 所属模块）
   - 文件路径索引（RQ-XXX → sdd-1/RQ-XXX-*.md 位置）
   - 依赖关系（RQ-XXX 依赖于 RQ-YYY）
   - 状态标记（ACTIVE, DEPRECATED, ARCHIVED）
...
# 系统地图（SYSTEM_MAP.md）
## RQ-ID 索引
| RQ-ID | 模块 | 文件位置 | 依赖关系 | 状态 | 更新时间 |
|-------|------|--------|--------|------|---------|
| RQ-001 | auth | sdd-1/auth/RQ-001-oauth.md | - | ACTIVE | 2025-12-01 |
| RQ-010 | user | sdd-1/user/RQ-010-profile.md | RQ-001 | ACTIVE | 2025-12-01 |
| RQ-020 | payment | sdd-1/payment/RQ-020-gateway.md | RQ-001, RQ-010 | ACTIVE | 2025-11-30 |
```

**AI 的行为规范**：
- ✅ 每次 SDD-2 完成后自动更新
- ✅ 如果发现 SYSTEM_MAP 信息不准确，自动修复
- ✅ 不增加人类的维护成本
- ❌ 不能假设 SYSTEM_MAP 已经正确，必须主动验证

#### 17.5.2 定期维护审计（人类验证）

每季度进行一次上下文管理审计（由 ADMIN 或 PM 执行）：

```
【Q1 上下文审计清单】

□ SYSTEM_MAP.md 是否存在且最近更新时间 < 30 天
□ SYSTEM_MAP.md 中的所有 RQ-ID 是否都有对应的 SDD 文档
□ 是否有孤立的 RQ（在 sdd-1/ 中存在但不在 SYSTEM_MAP 中）
□ 模块划分是否合理（避免过度细分或过度聚合）
□ 是否有 DEPRECATED 的 RQ 需要推进到 ARCHIVED
□ 文件大小是否合理（单个 RQ 文档 < 50 KB）
□ 是否有可以归档的已废弃 RQ（> 6 个月未维护）

【修复行动】
- 如果 SYSTEM_MAP.md 缺失或过时 → VA 立即重建
- 如果发现孤立 RQ → 确认是否应当删除或重新编目
- 如果发现文件过大 → 拆分为多个 RQ
```

---

## 18. 紧急豁免协议（Emergency Waiver Protocol）

> **目的**：在线上 P0 级故障需要紧急修复时，允许 ADMIN 临时绕过合规流程，但需要自动创建相应的技术债条目进行后续清理。
...
| 条件 | 说明 |
|-----|------|
| **严重级别** | P0 级（用户无法使用、数据丢失、安全事件） |
| **时间压力** | 需要在 1 小时内修复 |
| **影响范围** | 涉及生产环境核心功能 |
| **无其他选择** | 已确认无法通过热启动、配置变更等快速方案解决 |
...
```
[ADMIN_OVERRIDE_EMERGENCY]
严重级别：P0（线上支付系统宕机）
触发时间：2025-01-20 14:30:00 UTC
原因说明：数据库连接池耗尽，导致所有支付请求超时
修复预计：添加应急连接数限流，预计 20 分钟
...
当 ADMIN 发出 `[ADMIN_OVERRIDE_EMERGENCY]` 指令时：
...
✅ 立即执行：
- 放行代码审查，允许跳过 SDD-2 审查
- 跳过流程合规检查（不需要走正式的 SDD-0/1/2）
- 允许代码直接合并到 main

❌ 不跳过的项目：
- 【安全底线】：OWASP Top 10 关键漏洞仍需检查
- 【数据完整性】：事务一致性仍需保证

⚠️  创建技术债条目：
在 inboxes/ 中创建一个 P0 级的后续清理任务
标题：DEBT-20250120-PaymentHotfix-Cleanup
```
...
仍需检查的项目：
⚠️  【安全性】：无 SQL 注入、XSS、认证绕过
⚠️  【逻辑正确性】：修复是否真的解决了问题
⚠️  【无引入新 bug】：改动是否可能触发其他故障

标记规范：
在代码中添加注释：
// HOTFIX-DEBT: 缺少单元测试，需在 3 天内补全
// Tracked in: DEBT-20250120-001
```

### 18.4 自动技术债创建

当豁免指令被执行时，系统（或 AI）**必须自动**创建一个技术债条目：

#### 技术债格式：
```markdown
# 技术债：支付系统紧急修复补全

**债务 ID**：DEBT-20250120-001
**关联 Hotfix**：Commit 1a2b3c4d
**创建时间**：2025-01-20 14:30:00
**Deadline**：2025-01-23 18:00:00 （T+3 天）
**优先级**：P0
**所有者**：DEV/Payment Team
...
### 第二阶段（T+24h）：测试补全
- [ ] 补充本次修复涉及代码的单元测试（目标覆盖率 ≥ 85%）
- [ ] 补充集成测试，覆盖完整的支付流程
- [ ] 补充压力测试，验证连接池的容量预估是否合理
...
线上故障：支付系统连接池耗尽，导致所有支付请求超时。

应急修复：添加连接池大小限流，防止过载。

长期方案待定：需在 T+3 天内评估是否需要扩大连接池、升级中间件、或优化查询。
...
□ 本月豁免次数是否 > 2？
  如果是，说明可靠性有问题，需要分析根本原因

□ 是否有豁免的技术债超期未清？
  如果是，自动升级为高优先级任务

□ 豁免内容是否真的满足"P0 级"标准？
  如果否，对 ADMIN 进行警告，滥用豁免将扣除绩效分

建议：每季度豁免 ≤ 1 次是健康的频率。
      如果频繁（每月 > 3 次），说明前期设计或测试的流程需要改进。
```
